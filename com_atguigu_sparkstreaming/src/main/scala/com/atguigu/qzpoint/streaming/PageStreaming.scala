...
/**
  * 页面转换率实时统计
  *
  * nohup spark2-submit  --master yarn --deploy-mode client --driver-memory 1g --num-executors 2 --executor-cores 2 --executor-memory 2g --class com.atguigu.qzpoint.streaming.PageStreaming  --queue spark com_atguigu_sparkstreaming-1.0-SNAPSHOT-jar-with-dependencies.jar >1.txt>&1 &
  *
  * 如何查看worker端日志，我们现在是将driver端日志保存在1.txt里边，可以直接tail  -f 实时观看，但是worker端日志怎么看？
  *   我这个任务运行的webui上，stage里边，看到有一个task里边。有两个选项
  *     stdout  这个不是日志，这个是，如果我们在代码在worker端进行打印了。可以在这个stout里边看。
  *     stderr  如果想要看worker端打印的日志的话，在这个里边看，这里会显示整个worker端日志。比如：你这个sparkstreaming去操作mysql报错了。
  *     在我们linux命令行，监听的tail-f 是看不到的。我们的提交模式是yarnclient。
  *     我们的sparkstreaming任务，如果要停止的话，现在在yarn上跑的任务我想停掉，怎么做？任务是分布式的，直接kill是不行的。
  *     需要通过yarn的kill。
  *       yarn application -kill applicationid（在yarn上web端看看）
  *
  *     然后我们的这个任务，要放到集群上去提交的话，cdh上 spark2配置里边kafka版本选择0.10.
  */
object PageStreaming {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //这个方法是一个广播文件的作用，driver端去读取到这个路径下的文件内容。然后调用addFile方法，将这个文件广播给每一个worker端。
