...
/**
  *   并不是所有的业务都需要手动维护偏移量的。如果你的业务场景允许数据丢失的话，就是我这个业务场景丢一两条数据是没关系的。
  *   "enable.auto.commit" -> (false: lang.Boolean)  设置成true是没有问题的。  思维不要被限制死。
  *   --supervise 这个参数，重启spark任务用的。sparkstreaming放到yarn上去跑的话，正常情况下是一个运行的，但是如果中途挂掉了。
  *   而我们又不可能人为的去盯着这个任务。可以通过这个参数，在提交的时候设置这个参数。他就会判断出你这个sparkstream是为正常退出的。
  *   就会把你你这个sparkstreaming任务重新拉起来。通过这个参数的。也有些公司不是通过这个参数的。它有类似于这个参数的linux进程的工具
  *   专门去监控linux进程，专门将挂掉的进程拉起来。
  *   面试官可能问你，你这个spark任务挂掉了怎么办？可以这么回答他。flink中也是通过设置参数，自动重启策略的。
  *   不过这个参数一般是运维人员去配置的。老师之前就是有这样一个工具的，专门监控linux进程的。
  */
